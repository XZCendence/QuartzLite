...
// Quartz: optional backdrop painted immediately beneath a window's background
// fill, used to put a blurred "glass" surface behind popups, menus, tooltips and
// translucent windows. Called from RenderWindowDecorations with the same rect,
// rounding and corner flags ImGui is about to fill, so the two line up exactly.
// Null by default — inert unless the application installs one.
typedef void (*ImGuiWindowBackdropFn)(ImGuiWindow* window, ImDrawList* draw_list, const ImRect& rect,
                                      float rounding, ImDrawFlags rounding_flags, ImU32 bg_col);
extern IMGUI_API ImGuiWindowBackdropFn GImGuiWindowBackdrop;
...
//#endif
    ...
    // [Quartz] Accent edge for widget frames only (see RenderFrame in imgui.cpp). Pass NULL to
    // restore the stock ImGuiCol_Border behaviour. Does not affect window/child/popup borders.
    IMGUI_API void          SetFrameEdgeColor(const ImVec4* col);
