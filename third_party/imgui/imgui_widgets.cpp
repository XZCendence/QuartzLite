...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        if (highlighted || selected)
        {
            ...
            // [Quartz] Give the highlight the same treatment as a tab: accent
            // outline around a darker fill, rather than a flat accent slab. The
            // fill comes from the Header* colours above (dimmed in the theme);
            // this adds the edge, which upstream RenderFrame omits (border=false).
            // ImGuiCol_TabSelectedOverline is this theme's purest accent value and
            // is already the source for the tab outlines. Held back to a fraction
            // of it for the same reason DrawInactiveTabOutlines uses TabBorderAlpha:
            // at full strength the edge outshines the row it is meant to frame.
            window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_TabSelectedOverline, 0.50f),
                                      style.SelectableRounding, 0, 1.0f);
        }
        if (g.NavId == id)
        {
            ImGuiNavRenderCursorFlags nav_render_cursor_flags = ImGuiNavRenderCursorFlags_Compact;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (is_visible)
    {
        ...
        const ImU32 tab_col = held
            ? GetColorU32(ImLerp(style.Colors[ImGuiCol_TabSelected], style.Colors[ImGuiCol_TabSelectedOverline], 0.32f))
            : GetColorU32(hovered ? ImGuiCol_TabHovered : tab_contents_visible ? (tab_bar_focused ? ImGuiCol_TabSelected : ImGuiCol_TabDimmedSelected) : (tab_bar_focused ? ImGuiCol_Tab : ImGuiCol_TabDimmed));
        TabItemBackground(display_draw_list, bb, flags, tab_col);

        // Quartz's document outline: an open bottom lets the selected tab join
        // its content, while inactive and hover states retain clear geometry.
        const bool emphasized = tab_contents_visible || hovered || held;
        const ImU32 outline_col = tab_contents_visible
            ? GetColorU32(tab_bar_focused ? ImGuiCol_TabSelectedOverline : ImGuiCol_TabDimmedSelectedOverline)
            : GetColorU32((hovered || held) ? ImGuiCol_SeparatorHovered : ImGuiCol_Border);
        const float rounding = ImMax(0.0f, ImMin(style.TabRounding, bb.GetWidth() * 0.5f - 1.0f));
        const float top_y = bb.Min.y + 0.5f;
        const float bottom_y = bb.Max.y - style.TabBarBorderSize;
        display_draw_list->PathLineTo(ImVec2(bb.Min.x + 0.5f, bottom_y));
        if (rounding > 0.0f)
        {
            display_draw_list->PathArcToFast(ImVec2(bb.Min.x + rounding + 0.5f, top_y + rounding), rounding, 6, 9);
            display_draw_list->PathArcToFast(ImVec2(bb.Max.x - rounding - 0.5f, top_y + rounding), rounding, 9, 12);
        }
        else
        {
            display_draw_list->PathLineTo(ImVec2(bb.Min.x + 0.5f, top_y));
            display_draw_list->PathLineTo(ImVec2(bb.Max.x - 0.5f, top_y));
        }
        display_draw_list->PathLineTo(ImVec2(bb.Max.x - 0.5f, bottom_y));
        display_draw_list->PathStroke(outline_col, emphasized ? 1.5f : 1.0f);
